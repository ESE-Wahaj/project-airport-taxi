...
export const fetchCar  = async  (id) => {
  const token = Cookies.get('authToken');
  try {
    const response = await axios.post(`${host}/api/car-by-Id`,{ id}, {
      withCredentials: true,
      headers: {
        Authorization: `Bearer ${token}`,
      },
...
export const deleteCar  = async  (id, navigate) => {
  const token = Cookies.get('authToken');
  console.log(token)
  try {
    const response = await axios.delete(`${host}/api/delete-car`, {
      withCredentials: true,
      headers: {
        Authorization: `Bearer ${token}`,
      },
      ...
      icon: "success",
      title: "Success",
      text: response.data.message || "Car deleted successfully!",
    }).then((result) => {
...
export const AddCar = async (carData, navigate) => {
  const token = Cookies.get('authToken');
  try {
    const response = await axios.post(`${host}/api/add-car`, carData, {
      headers: {
        'Authorization': `Bearer ${token}`,
      }
    });
    Swal.fire({
      icon: "success",
      title: "Success",
      text: response.data.message || "Car Added successfully!",
    }).then((result) => {
      if (result.isConfirmed) {
        // Reload the page to get the latest data after the user clicks "OK"
        navigate(`/admin/dashboard`);
    ...
    // throw new Error("Error Adding the car");
  }
}
export const updateBookingStatus = async (formdata) => {
  const token = Cookies.get('authToken');
  try {
    const response = await axios.patch(`${host}/api/update-status`, formdata, {
      headers: {
        'Authorization': `Bearer ${token}`,
      }
    });
    Swal.fire({
      icon: "success",
      title: "Success",
      text: response.data.message || "booking updated successfully!",
    });
  } catch (error) {
    throw new Error("Error updating the booking status");
  }
}
export const updateCar = async (carData,navigate ) => {
  const token = Cookies.get('authToken');
  try {
...
export const getAllBookings = async () => {
  const token = Cookies.get('authToken');
  try {
    const response = await axios.get(`${host}/api/allbookings`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        }
