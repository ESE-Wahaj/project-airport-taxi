...
func (s *Server) UpdateBookingStatus(c *gin.Context) {
	...
	var request models.UpdateBookingStatus
	if err := c.ShouldBindJSON(&request); err != nil {
		c.JSON(http.StatusBadRequest, models.ErrorResponse{Error: err.Error()})
		return
	}

	err := s.api.UpdateBookingStatusAPI(c, &request)
	if err != nil {
		c.JSON(http.StatusInternalServerError, models.ErrorResponse{Error: "Unable to update booking status"})
		return
	}
