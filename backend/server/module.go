...
func NewServerImpl(r *gin.Engine) *gin.Engine {
	...
	base.DELETE("/delete-car", middlewares.Auth([]string{"admin"}), server.DeleteCar)
	base.POST("/bookme", server.Bookme)
	base.POST("/car-by-Id", middlewares.Auth([]string{"admin"}), server.GetCarById)
	...
	base.GET("/allbookings", middlewares.Auth([]string{"admin"}), server.GetAllBookings)
	...
	base.PATCH("/update-status", middlewares.Auth([]string{"admin"}), server.UpdateBookingStatus)
	return r
}
